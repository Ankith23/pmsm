...
void main(void)
{
    ...
    EDIS;
    while(1){
        GpioDataRegs.GPADAT.bit.GPIO31 = GpioDataRegs.GPADAT.bit.GPIO2;
        GpioDataRegs.GPBDAT.bit.GPIO34 = GpioDataRegs.GPADAT.bit.GPIO4;
...
////        if(GpioDataRegs.GPATOGGLE.bit.GPIO31 == 1) GpioDataRegs.GPBTOGGLE.bit.GPIO34 = 1;
          ...
          EPwm3Regs.TBPHS.bit.TBPHS = (11160);          // Phase is 0
//          if((j+=1)==200) j=0;
...
//       if(pos3>360) pos3 = pos3 - 360;
   ...
   New_Prd=(Period4/12);
    ...
    EPwm6Regs.CMPA.bit.CMPA=(0.5*New_Prd)*((1+sin((pos*3.14)/180) + 4*3.14/3));
    EPwm5Regs.CMPA.bit.CMPA=(0.5*New_Prd)*(1+sin(((pos*3.14)/180) + 2*3.14/3));
    EPwm4Regs.CMPA.bit.CMPA=(0.5*New_Prd)*(1+sin(((pos*3.14)/180)));

    EPwm6Regs.CMPB.bit.CMPB=(0.5*New_Prd)*((1+sin((pos*3.14)/180) + 4*3.14/3));
    EPwm5Regs.CMPB.bit.CMPB=(0.5*New_Prd)*(1+sin(((pos*3.14)/180) + 2*3.14/3));
        ...
        GpioCtrlRegs.GPDMUX1.bit.GPIO111=00;// by muxed pins we have to select pwm value
         ...
         GpioCtrlRegs.GPBMUX2.bit.GPIO52=00;// by muxed pins we have to select pwm value
          ...
          GpioCtrlRegs.GPCMUX2.bit.GPIO94=00;// by muxed pins we have to select pwm value
           ...
           GpioCtrlRegs.GPAGMUX2.bit.GPIO31 = 00;
           GpioCtrlRegs.GPAMUX2.bit.GPIO31 = 00;
           GpioCtrlRegs.GPADIR.bit.GPIO31 = 01;

           GpioCtrlRegs.GPBGMUX1.bit.GPIO34 = 00;
           GpioCtrlRegs.GPBMUX1.bit.GPIO34 = 00;
           GpioCtrlRegs.GPBDIR.bit.GPIO34 = 01;
...
void pwm2(void){
    EALLOW;
      ...
      CpuSysRegs.PCLKCR0.bit.TBCLKSYNC = 1;
      EPwm2Regs.TBCTL.bit.CLKDIV = 7;          //Divides the EPWMCLK by 1 (3 bits - binary)
       EPwm2Regs.TBCTL.bit.HSPCLKDIV = 7;       //Divides the EPWMCLK by 1 : TBCLK = EPWMCLK / (HSPCLKDIV x CLKDIV)

       EPwm2Regs.TBCTL.bit.CTRMODE = 1;        // 0 for up counter
...
void pwm3(void){
    EALLOW;
      ...
      CpuSysRegs.PCLKCR0.bit.TBCLKSYNC = 1;
      EPwm3Regs.TBCTL.bit.CLKDIV = 7;          //Divides the EPWMCLK by 1 (3 bits - binary)
       EPwm3Regs.TBCTL.bit.HSPCLKDIV = 7;       //Divides the EPWMCLK by 1 : TBCLK = EPWMCLK / (HSPCLKDIV x CLKDIV)

       EPwm3Regs.TBCTL.bit.CTRMODE = 1;        // 0 for up counter
       EPwm3Regs.TBCTR = 0x0000;              // Clear counter

       EPwm3Regs.CMPB.bit.CMPB = 500;          // EPWM compare value

       EPwm3Regs.AQCTLA.bit.CAD = 1;           // Action qualifier
...
void pwm4(void)
{
    EALLOW;
         ...
         EPwm4Regs.TBCTL.bit.HSPCLKDIV=7;
        EPwm4Regs.TBPRD=4464;
        EPwm4Regs.CMPA.bit.CMPA = 2232;
         //double check//
...
//    void pwgen(void)
//    {
//    Period_I = (25000000/frequ);    //    Period_I = 178571/fre;                       //((80000000/128)/14) = 44342
...
//
////    }
    EPwm2Regs.TBPRD = 44642;                 // EPWM 2
         EPwm2Regs.CMPA.bit.CMPA = 22321;          // EPWM compare value

         EPwm3Regs.TBPRD = 44642;                 // EPWM 2
               EPwm3Regs.CMPA.bit.CMPA = 22321;          // EPWM compare value
