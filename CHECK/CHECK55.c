...
long Pre_Value,Curr_Value,Timer_Value, Period_I, Period_A, Period_B,Period_S, Phase_B,Phase_S,latchvalue[360],Pr[360],Py[360],Pz[360],H1,H2,H3,prd,a;
...
void main(void)
{
   ...
   InitTimer0();
    ...
    while(1){
        Fre_Sum = 0;
          equal();
//          Fre_Average = Fre_Sum*0.02;
//                     fre = ((124100273.00/Fre_Average));//1230769.2308/const
          pwgen();
//          BLDCMOTOR();
//          pos = (EQep1Regs.QPOSSLAT)/4;
...
//if(EQep1Regs.QFLG.bit.SEL==1)
{
          if((j+=1)==360) j=0;
             ...
             pos3 = pos + 240;     //Phase R
             if(pos2>360) pos2 = pos2 - 360;
             if(pos3>360) pos3 = pos3 - 360;

//             EQep1Regs.QCLR.bit.SEL=1;
             }
            D6 = 0.5*(1+sin((pos3*3.14)/180));
            D5 = 0.5*(1+sin((pos2*3.14)/180));
            D4 = 0.5*(1+sin((pos*3.14)/180));
...
//             EPwm1Regs.TBPHS.bit.TBPHS = 0;
             ...
             else if (frequency > 990.0 && frequency < 7000)// For Frequency more than 10Hz Run as PMSM Motor
             {
                BLDC=0;
                PMSM = 0;
...
//       if(pos3>360) pos3 = pos3 - 360;
    /**************************************************** EPWM PRESCALAR CONFIGURATION ****************************************************/
     ...
     EPwm6Regs.TBCTL.bit.HSPCLKDIV=1;
     /**************************************************** EPWM PRESCALAR CONFIGURATION ****************************************************/
       Period4 = Period_A;       //recalculation of index pulses
   New_Prd=(Period4*2);
    EPwm4Regs.TBPRD=New_Prd;
    EPwm5Regs.TBPRD=New_Prd;
    EPwm6Regs.TBPRD=New_Prd;
...
//             Fre_Sum = FreArray[0] + FreArray[1] + FreArray[2] + FreArray[3] + FreArray[4] + FreArray[5] + FreArray[6] + FreArray[7] + FreArray[8] + FreArray[9] + FreArray[10] + FreArray[0] + FreArray[11] + FreArray[12] + FreArray[13] + FreArray[14] + FreArray[15] + FreArray[16] + FreArray[17] + FreArray[18] + FreArray[19] + FreArray[20] + FreArray[21] + FreArray[22] + FreArray[23] + FreArray[24] + FreArray[25] + FreArray[26] + FreArray[27] + FreArray[28] + FreArray[29] + FreArray[30];
         }

         Fre_Average = Fre_Sum*0.01;
...
//    Period_I = 178571/fre;                                //((80000000/128)/14) = 44342
     EPwm1Regs.TBPRD = (Period_I/29);                       // using as strobe for 12 degree resolution
     EPwm1Regs.CMPA.bit.CMPA = (Period_I/348);              // EPWM compare value


     Period_A = Period_I/180;                               // Generation of 180 EQEP1A Pulses
     ...
     Period_B = Period_I/180;                               // Generation of 180 EQEP1B Pulses
