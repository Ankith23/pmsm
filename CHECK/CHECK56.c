...
//             EPwm1Regs.TBPHS.bit.TBPHS = 0;
             ...
             else if (frequency > 990.0 && frequency < 7000)
             {
                BLDC=0;
                PMSM = 0;
                NEWBLDCMOTOR();
...
{
    {
     ...
     if((Curr_Value == 1)&(Pre_Value == 0))
     {
         ...
         if((Curr_Value == 1)&(Pre_Value == 0))
         {
             ...
             fre = (125637931/(((CpuTimer0Regs.PRD.all)-Timer_Value)));
             CpuTimer0Regs.TCR.bit.TRB = 1;
         }
...
void pwgen(void)
{
    Period_I = (500000000/frequency);                       //((80000000/128)/14) = 44342
//    Period_I = 178571/fre;                       //((80000000/128)/14) = 44342
     EPwm1Regs.TBPRD = (Period_I/29);                 // using as strobe for 8 degree resolution
     EPwm1Regs.CMPA.bit.CMPA = (Period_I/348);   // EPWM compare value


     Period_A = Period_I/180;
